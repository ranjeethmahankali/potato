#pragma once
/*
  Copy pasted from https://github.com/bitmeal/argv_split
...
private:
  ...
  void splitStrToVectorBy(const std::string&        str,
                          char                      delim,
  ...
  void _parse(const std::string& cmdlinestr)
  {
    ...
    size_t w_front = cmdlinestr.size();     // if no quotes are found this is used as the
                                            // substring SIZE for splitting by space
    size_t w_back = cmdlinestr.size() - 1;  // if no quotes are found this is used as
                                            // OFFSET for the next (and final) recursion

    size_t q_front = findFirstQuote(cmdlinestr);
    size_t q_back  = findMatching(cmdlinestr, q_front);
    // found an unescaped qoute, are same when npos an so no quote found. are different
    // when two quotes or only a first one are found
    if (q_front != q_back) {
      // get quoted string
      // and get attached string after quoted string. open quotes will be treated as
      // quotes til the end!
      ...
      // did not find matching quote further in string, everything til the end is quoted
      // now
      else {
...
public:
  argv_split()
      : quotes {SGLQUOTE_HEX_CODE, DBLQUOTE_HEX_CODE, '\0'}
      , space {SPACE_HEX_CODE, '\0'}
      , prependProgname(false)
  {}

  explicit argv_split(const std::string& progname)
      : argv_split()
  {
    prependProgname = false;
  ...
  void include_progname() { prependProgname = true; }
